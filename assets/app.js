...
function renderMacros(recipe) {
      ...
      return `
        <div class="macro-row">
          <span class="macro-row__name">
            ${escapeHtml(person.name)}${person.isBase ? " (Base)" : ""}
          </span>
  ...
  return `
    <section class="recipe-section">
      <h3 class="recipe-section-title">✦ Macros por persona</h3>
    ...
    .map((ingredient) => {
      ...
      const personCells = nonBasePeople
        .map((person) => {
          ...
          const extraText =
            extra > 0
              ? ` <span class="ingredient-extra">(+${formatIngredientQuantity(
                  extra,
                  ingredient.unit
                )})</span>`
              : "";
      ...
      const total = formatIngredientQuantity(
      ...
      return `
        <tr>
          ...
          ${personCells}
          <td class="total-quantity">${escapeHtml(total)}</td>
        </tr>
      `;
    })
    .join("");
...
function calculateIngredientTotal(ingredient) {
  ...
  return state.plan.people.reduce((total, person) => {
    return total + calculatePersonIngredientQuantity(ingredient, person.id);
  }, 0);
}
